...
const startDualscreenSession = () => call("start_dualscreen_session");
...
const uk = {
    ...
    "Dual-screen emulator mode": "Двоекранний режим емулятора",
    "Restarts into a session that drives BOTH displays at once (game mode itself can only use one): the emulator's top screen goes to the external display, the bottom one - with touch - stays on the internal panel. Made for Azahar (3DS) and melonDS (DS). Quitting the emulator returns to game mode.": "Перезапускає в сесію, що працює з ОБОМА дисплеями водночас (ігровий режим уміє лише один): верхній екран емулятора йде на зовнішній дисплей, нижній - із тачем - лишається на вбудованому. Зроблено для Azahar (3DS) і melonDS (DS). Вихід з емулятора повертає в ігровий режим.",
    "Start Dual-Screen Session": "Запустити двоекранну сесію",
    "Dual-screen mode": "Двоекранний режим",
    "Launch in the dual-screen session (internal + external display) instead of game mode - for dual-screen emulators like Azahar/melonDS. Requires the external display connected.": "Запускати гру у двоекранній сесії (внутрішній + зовнішній дисплей) замість ігрового режиму — для двоекранних емуляторів на кшталт Azahar/melonDS. Потрібен підключений зовнішній дисплей.",
    ...
    "Speed": "Швидкість",
    "Updates per second": "Оновлень на секунду",
    "/s": "/с",
...
const ru = {
    ...
    "Dual-screen emulator mode": "Двухэкранный режим эмулятора",
    "Restarts into a session that drives BOTH displays at once (game mode itself can only use one): the emulator's top screen goes to the external display, the bottom one - with touch - stays on the internal panel. Made for Azahar (3DS) and melonDS (DS). Quitting the emulator returns to game mode.": "Перезапускает в сессию, работающую с ОБОИМИ дисплеями сразу (игровой режим умеет лишь один): верхний экран эмулятора уходит на внешний дисплей, нижний - с тачем - остаётся на встроенном. Для Azahar (3DS) и melonDS (DS). Выход из эмулятора возвращает в игровой режим.",
    "Start Dual-Screen Session": "Запустить двухэкранную сессию",
    "Dual-screen mode": "Двухэкранный режим",
    "Launch in the dual-screen session (internal + external display) instead of game mode - for dual-screen emulators like Azahar/melonDS. Requires the external display connected.": "Запускать игру в двухэкранной сессии (внутренний + внешний дисплей) вместо игрового режима — для двухэкранных эмуляторов вроде Azahar/melonDS. Требуется подключённый внешний дисплей.",
    ...
    "Speed": "Скорость",
    "Updates per second": "Обновлений в секунду",
    "/s": "/с",
...
const es = {
    ...
    "Dual-screen emulator mode": "Modo emulador a doble pantalla",
    "Restarts into a session that drives BOTH displays at once (game mode itself can only use one): the emulator's top screen goes to the external display, the bottom one - with touch - stays on the internal panel. Made for Azahar (3DS) and melonDS (DS). Quitting the emulator returns to game mode.": "Reinicia en una sesión que usa AMBAS pantallas a la vez (el modo de juego solo puede una): la pantalla superior del emulador va a la externa y la inferior - con táctil - queda en la interna. Pensado para Azahar (3DS) y melonDS (DS). Salir del emulador vuelve al modo de juego.",
    "Start Dual-Screen Session": "Iniciar sesión de doble pantalla",
    "Dual-screen mode": "Modo de doble pantalla",
    "Launch in the dual-screen session (internal + external display) instead of game mode - for dual-screen emulators like Azahar/melonDS. Requires the external display connected.": "Iniciar el juego en la sesión de doble pantalla (pantalla interna + externa) en lugar del modo de juego — para emuladores de doble pantalla como Azahar/melonDS. Requiere la pantalla externa conectada.",
    ...
    "Speed": "Velocidad",
    "Updates per second": "Actualizaciones por segundo",
    "/s": "/s",
...
const fr = {
    ...
    "Dual-screen emulator mode": "Mode émulateur double écran",
    "Restarts into a session that drives BOTH displays at once (game mode itself can only use one): the emulator's top screen goes to the external display, the bottom one - with touch - stays on the internal panel. Made for Azahar (3DS) and melonDS (DS). Quitting the emulator returns to game mode.": "Relance dans une session utilisant les DEUX écrans à la fois (le mode jeu n'en gère qu'un) : l'écran du haut de l'émulateur va sur l'externe, celui du bas - tactile - reste sur l'interne. Pensé pour Azahar (3DS) et melonDS (DS). Quitter l'émulateur ramène au mode jeu.",
    "Start Dual-Screen Session": "Lancer la session double écran",
    "Dual-screen mode": "Mode double écran",
    "Launch in the dual-screen session (internal + external display) instead of game mode - for dual-screen emulators like Azahar/melonDS. Requires the external display connected.": "Lancer le jeu dans la session double écran (écran interne + externe) au lieu du mode jeu — pour les émulateurs double écran comme Azahar/melonDS. Nécessite l'écran externe branché.",
    ...
    "Speed": "Vitesse",
    "Updates per second": "Mises à jour par seconde",
    "/s": "/s",
...
function SliderEdit({ label, value, min, max, step, onChange, format, valueSuffix }) {
...
function Display(_props) {
    ...
    return (SP_JSX.jsxs(DFL.PanelSection, { title: t("External Display"), children: [SP_JSX.jsx(SelectEdit, { label: t("Primary Display"), value: selectedConnector, options: primaryOptions, onChange: selectPrimary, disabled: saving }), state.useExternal && (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(SelectEdit, { label: t("Resolution"), value: currentMode, options: modeOptions, onChange: selectMode, disabled: saving || activeDisconnected }), SP_JSX.jsx(SelectEdit, { label: t("Rotation"), value: state.orientation, options: ORIENTATION_OPTIONS, onChange: selectOrientation, disabled: saving || activeDisconnected }), isPortrait(state.width, state.height) && (SP_JSX.jsx(DFL.Field, { label: t("This is a portrait panel - pick the rotation that makes the image upright. Applied on game mode restart.") })), SP_JSX.jsx(ToggleRow, { label: t("Internal screen as touchpad"), description: t("While an external display is primary, the dark internal touchscreen works as a trackpad (correct orientation, tap = click). Off: it is disabled entirely."), value: state.internalTouchpad, onChange: toggleTouchpad, disabled: saving })] })), externals.length === 0 && (SP_JSX.jsx(DFL.Field, { label: t("No external display detected. Connect one (dock/USB-C/HDMI) to choose it here.") })), activeDisconnected && (SP_JSX.jsx(DFL.Field, { label: t("This display isn't connected right now - game mode runs on the internal screen until it's plugged back in. Its settings are remembered.") })), errorMessage && SP_JSX.jsx(DFL.Field, { label: t("Error: {message}", { message: errorMessage }) }), externals.some((c) => c.connected) && (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(DFL.Field, { label: t("Dual-screen emulator mode"), description: t("Restarts into a session that drives BOTH displays at once (game mode itself can only use one): the emulator's top screen goes to the external display, the bottom one - with touch - stays on the internal panel. Made for Azahar (3DS) and melonDS (DS). Quitting the emulator returns to game mode.") }), SP_JSX.jsx("div", { className: "nebel-reset-row", children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: restarting, onClick: () => {
                            ...
                            }, children: t("Start Dual-Screen Session") }) })] })), SP_JSX.jsx("div", { className: "nebel-reset-row", children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: restarting, onClick: () => {
                        setRestarting(true);
                        setErrorMessage("");
...
function Lighting({ config, setConfig, qam }) {
    ...
    return (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsxs(DFL.PanelSection, { title: t("Stick Lighting"), children: [SP_JSX.jsx(ToggleRow, { label: t("Enable"), description: t("Turn both sticks off entirely, without losing the mode/color settings below"), value: stickLed.enabled, onChange: setStickLedEnabled$1 }), !stickLed.enabled && SP_JSX.jsx(DFL.Field, { label: t("Sticks are off - settings below are kept, not applied.") }), stickLed.enabled && (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(SelectEdit, { label: t("Mode"), value: mode, options: MODE_OPTIONS, onChange: setStickLedMode$1 }), !qam && (SP_JSX.jsxs(SP_JSX.Fragment, { children: [mode === "spin" && (SP_JSX.jsx(ToggleRow, { label: t("Soft trail"), description: t("Trailing fade (uses Size below) instead of a single hard-edged dot"), value: !!sideState.chase, onChange: setStickLedChase$1 })), mode === "reactive" && (SP_JSX.jsx(ToggleRow, { label: t("Compass"), description: t("Point the lit zone(s) at the stick's push direction instead of lighting evenly"), value: !!sideState.compass, onChange: setStickLedCompass$1 })), mode === "duotone" && (SP_JSX.jsx(ToggleRow, { label: t("Seesaw"), description: t("Breathe the two color groups against each other instead of a static split"), value: !!sideState.seesaw, onChange: setStickLedSeesaw$1 })), Object.entries(PARAM_UI)
                                        ...
                                        return (SP_JSX.jsx(SliderEdit, { label: hz ? t("Updates per second") : spec.label, value: hz ? Math.round(spec.fromBackend(raw) / 10) : spec.fromBackend(raw), min: hz ? 3 : spec.min, max: hz ? 30 : spec.max, step: hz ? 1 : spec.step, valueSuffix: hz ? t("/s") : undefined, onChange: (value) => setStickLedParam$1(param, spec.toBackend(hz ? value * 10 : value)) }, param));

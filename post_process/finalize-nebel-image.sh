...
#
# All supported devices (SM8250/SM8550/SM8650/SM8750) boot via ROCKNIX ABL's
# per-device DTB picker from the /KERNEL Android bootimg on the ESP. GRUB is
# no longer used. (Port of the nebel-line "drop GRUB" rework, 249e8bc.)

set -euxo pipefail

RAW_IMAGE="${1:-output/raw/disk.raw}"
ROCKNIX_ABL_VERSION="${ROCKNIX_ABL_VERSION:-v1.1.7}"
OUT="${OUT:-output/nebel-$(TZ='America/New_York' date +%Y%m%d).img.gz}"
...
# Disable the GRUB fallback entirely; ABL boots /KERNEL directly and picks the
# right DTB itself for all supported SoCs.
if [ -d "${WORK}/mnt/EFI" ]; then
    sudo mv "${WORK}/mnt/EFI" "${WORK}/mnt/EFI.disabled"
fi
sudo sync
sudo umount "${WORK}/mnt"

# Android shows this label when copying the ABL
sudo fatlabel "${ESP}" NEBEL

...
UPDATE="$ROOT/system_files/usr/libexec/nebel/nebel-abl-update"
FINALIZE="$ROOT/system_files/usr/libexec/nebel/nebel-abl-finalize"
VERSION_TOOL="$ROOT/system_files/usr/lib/nebel/abl-version"
WORK="$(mktemp -d)"
...
TARGET="$WORK/target"
PAYLOAD_DIR="$TARGET/usr/lib/nebel/abl"
...
[[ "$(NEBEL_ABL_RELEASES="$catalog" python3 "$VERSION_TOOL" --with-soc "$payload")" == "1.1.8 SM8550" ]] ||
    fail "hash catalog did not identify target payload"
[[ "$(NEBEL_ABL_RELEASES="$catalog" python3 "$VERSION_TOOL" --lookup 1.1.8 SM8550)" == "8192 ${target_hash}" ]] ||
    fail "catalog lookup did not return target metadata"

write_manifest() {
    local auto=$1
    cat > "$PAYLOAD_DIR/manifest" <<EOF
NEBEL_ABL_VERSION=1.1.8
NEBEL_ABL_AUTO=${auto}
NEBEL_ABL_SHA256_SM8550=${target_hash}
EOF
}
write_manifest 1
...
if [[ "${input:-}" == "${NEBEL_TEST_PAYLOAD}" ]]; then
    printf '%s\n' "$output" >> "$NEBEL_TEST_WRITE_LOG"
    [[ "${NEBEL_TEST_DD_FAIL:-0}" != 1 ]] || exit 1
fi
exec /usr/bin/dd "$@"
EOF
chmod +x "$WORK/bin/dd"
...
base_env=(
    NEBEL_ABL_TESTING=1
    NEBEL_ABL_DEVICE_ID=retroid-pocket-5
    NEBEL_ABL_SOC=SM8550
    NEBEL_ABL_A="$ABL_A"
    NEBEL_ABL_B="$ABL_B"
    NEBEL_ABL_LOCK="$WORK/lock"
    NEBEL_TEST_PAYLOAD="$payload"
    NEBEL_TEST_WRITE_LOG="$WORK/writes"
    NEBEL_TEST_SPLASH_LOG="$WORK/splash-log"
    ABL_VERSION_TOOL="$VERSION_TOOL"
    NEBEL_ABL_RELEASES="$catalog"
    SPLASH="$WORK/splash"
    PATH="$WORK/bin:$PATH"
)
...
PY
[[ "$(NEBEL_ABL_RELEASES="$catalog" python3 "$VERSION_TOOL" --with-soc "$short_tail")" == "1.1.6 SM8550" ]] ||
...
set +e
out=$(env "${base_env[@]}" NEBEL_TEST_DD_FAIL=1 \
...
# Upstream also exercises armada-control's abl_auto_enabled settings actions
# here; nebel-control has no ABL settings API, so that block is not ported.
# The /etc/nebel/abl.conf override the finalizer reads is covered below.

FROOT="$WORK/finalize"; FBOOT="$FROOT/boot"; FSYS="$FROOT/sysroot"
mkdir -p "$FBOOT/loader/entries" "$FSYS/ostree/deploy/old" \
    "$FSYS/ostree/deploy/target/usr/lib/nebel/abl"
...
EOF
printf 'NEBEL_ABL_AUTO=1\n' > "$FSYS/ostree/deploy/target/usr/lib/nebel/abl/manifest"
...
finalize_env=(BOOTROOT="$FBOOT" SYSROOT="$FSYS"
    ARGS_FILE="$ROOT/system_files/usr/lib/nebel/bootimg-args"
    UPDATER="$FROOT/updater" NEBEL_FINALIZE_LOG="$FROOT/finalize-log"
...
rm -f "$FROOT/abl.conf" "$FSYS/ostree/deploy/target/usr/lib/nebel/abl/manifest"
: > "$FROOT/finalize-log"
env "${finalize_env[@]}" "$FINALIZE"
...
grep -q '^NEBEL_ABL_AUTO=0$' "$ROOT/abl/release.env" ||
    fail "release manifest enables automatic flashing"
sync_unit="$ROOT/system_files/usr/lib/systemd/system/nebel-bootimg-sync.service"
grep -q '^ExecStop=-/usr/libexec/nebel/nebel-abl-finalize$' "$sync_unit" ||
